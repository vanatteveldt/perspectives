...
def sort_terms(terms: Iterable[Cterm]) -> List[Cterm]:
...
def get_sentence(term: Cterm) -> int:
    tokens = [naf.get_token(tid) for tid in naf.get_dict_tokens_for_termid(term.get_id())]
...
def get_terms_in_sentence(naf: KafNafParser, sent: int) -> Iterable[Cterm]:
    ...
    return sort_terms(naf.get_term(tid) for tid in naf.map_tokens_to_terms(tokenids))
